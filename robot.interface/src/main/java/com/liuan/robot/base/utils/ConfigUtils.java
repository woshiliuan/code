package com.liuan.robot.base.utils;
...
public class ConfigUtils {
    ...
    public static Properties getProperties() {
        if (PROPERTIES == null) {
            synchronized (ConfigUtils.class) {
                if (PROPERTIES == null) {
                    String path = System.getProperty("mqxsdk.properties.file");
                    if (path == null || path.length() == 0) {
                        path = System.getenv("mqxsdk.properties.file");
                        if (path == null || path.length() == 0) {
                            path = "jdbc.properties";
                        }
                    }
                    PROPERTIES = ConfigUtils.loadProperties(path, true, false);
                }
            }
        }
        return PROPERTIES;
     ...
     * <code>dubbo.properties</code>, <code>METE-INF/conf/foo.properties</code>
     * @param allowMultiFile if <code>false</code>, throw
     * {@link IllegalStateException} when found multi file on the class path.
     * @param optional is optional. if <code>false</code>, log warn when
     * properties config file not found!s
     ...
     * </ul>
     * @throws IllegalStateException not allow multi-file, but multi-file exsit
     * on class path.
     */
    public static Properties loadProperties(String fileName, boolean allowMultiFile, boolean optional) {
        ...
        try {
            ...
            while (urls.hasMoreElements()) {
                ...
                if(url.toString().indexOf("properties")>0)
                list.add(url);
            }
        } catch (Throwable t) {
        ...
        if (list.size() == 0) {
            if (!optional) {
                logger.info("No " + fileName + " found on the class path.");
            }
            return properties;
        }
        ...
        if (1 == list.size()) {
            ...
            if(os.toLowerCase().startsWith("win")){  
                url = url.substring(6, url.indexOf(fileName));  
            }else if(os.toLowerCase().startsWith("linux")||os.toLowerCase().matches("^(mac).+")){
                url = url.substring(5, url.indexOf(fileName));  
            }
            
            logger.info("config load properties file url " + url);
            File file = new File(url);
            File[] fileAry = file.listFiles();
          
            try {
                for (File f : fileAry) {
                    if(f.getName().indexOf("properties")>-1&&os.toLowerCase().startsWith("win"))
                        list.add(new java.net.URL("file:\\"+f.getAbsolutePath()));
                    else if(f.getName().indexOf("properties")>-1&&os.toLowerCase().startsWith("linux")||os.toLowerCase().matches("^(mac).+"))
                        list.add(new java.net.URL("file:"+f.getAbsolutePath()));
                }
